...
export function SpotModal({ spotName, spotData, onClose }: SpotModalProps) {
  const mapContainer = useRef<HTMLDivElement>(null);
  const mapInstance = useRef<any>(null);

  useEffect(() => {
    if (!mapContainer.current || !spotData || !spotData.coords || !window.AMap) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/60 backdrop-blur-sm slide-up">
      <div className="bg-paper w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-white">
          <h3 className="text-xl font-bold text-ink">{spotName} 游览攻略</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 hide-scrollbar flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 flex flex-col gap-6">
             {spotData.tips && (
                <div>
                  <div className="text-sm font-bold text-stone mb-2 uppercase tracking-wider">注意事项</div>
          ...
          {/* Map Side */}
          <div className="flex-1 flex flex-col min-h-[400px] lg:min-h-[auto]">
            <div className="text-sm font-bold text-stone mb-2 uppercase tracking-wider">
               {spotData.routeTitle || '推荐游览路线'}
            </div>
            <div ref={mapContainer} className="flex-1 bg-cream rounded-2xl border border-border overflow-hidden"></div>
            {spotData.highlights && (
                <div className="mt-4 text-xs text-stone leading-relaxed bg-white p-3 rounded-xl border border-border">
                   <span className="font-bold text-ink">✨ 必看亮点：</span> {spotData.highlights}
                </div>
            )}
          </div>
        </div>
